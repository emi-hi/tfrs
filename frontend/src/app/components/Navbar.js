...
class Navbar extends Component {
  static updateContainerPadding() {
    ...
    document
      .getElementById("main")
      .setAttribute("style", `padding-top: ${totalSpacing}px;`);
  }
  ...
  UNSAFE_componentWillReceiveProps(newProps) {
    if (newProps.unreadNotificationsCount != null) {
      ...
      if (
        newProps.unreadNotificationsCount > 0 &&
        newProps.unreadNotificationsCount < 1000
      ) {
  ...
  render() {
        ...
        <div className='container-fluid'>
          <div id='navbar-main-items'>
            ...
            {CONFIG.COMPLIANCE_REPORTING.ENABLED &&
              ...
              this.props.loggedInUser.hasPermission(
                PERMISSIONS_SECURE_DOCUMENT_UPLOAD.VIEW
              ) &&
              !this.props.loggedInUser.isGovernmentUser && (
                <NavLink
                  // activeClassName="active"
          ...
          <div id='user-options'>
            {this.props.loggedInUser.displayName && (
              <div className='navbar-item' id='display-name'>
                <DropdownButton
                ...
                >
                  ...
                  <MenuItem
                    className='dropdown-hidden-item'
                    href={`/assets/files/Transportation_Fuels_Reporting_System_-_${
                      this.props.loggedInUser.isGovernmentUser
                        ? "IDIR"
                        : "BCeID"
                    }_Manual.pdf`}
            ...
            <a
              className='navbar-item'
              href={`/assets/files/Transportation_Fuels_Reporting_System_-_${
                this.props.loggedInUser.isGovernmentUser ? "IDIR" : "BCeID"
              }_Manual.pdf`}
              id='navbar-help'
    ...
    const CollapsedNavigation = (
      <div id='navbar' className='collapse navbar-collapse' role='navigation'>
        <a id='navigation-anchor' href='#navigation-anchor'>
                  ...
                  {this.props.loggedInUser.roles &&
                    !this.props.loggedInUser.isGovernmentUser && (
                      <span id='organization-balance'>
                        Credit Balance:{" "}
                        {numeral(
                          organization.organizationBalance.validatedCredits
                        ).format(NumberFormat.INT)}{" "}
                        (
                        {numeral(
                          organization.organizationBalance.deductions
                        ).format(NumberFormat.INT)}
                        )
                      </span>
                    )}
